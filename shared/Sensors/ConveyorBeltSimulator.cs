using Az220.Shared.Helpers;

namespace Az220.Shared.Sensors;

public class ConveyorBeltSimulator: SensorBase, ISensor
    {
        ...
        public double Vibration {get => vibration;}

    public override string MessageString => JsonConvert.SerializeObject(new {
        vibration = Vibration
    });
    public override string LogMessageString => JsonConvert.SerializeObject(new{
        vibration = Vibration,
    ...
    public override Message CreateMessage()
    {
        ...
        msg.Properties.Add("sensorId","VSTel");
        msg.Properties.Add("beltAlert", BeltAlert);
        return msg;
    }
    public override Message CreateLogMessage()
    {
        var msg = base.CreateLogMessage();
        msg.Properties.Add("sensorId","VSLog");
        msg.Properties.Add("beltAlert", BeltAlert);
        return msg;
    }
    protected override Action<DeviceClient, Microsoft.Extensions.Logging.ILogger?> DefaultSendAction => 
    (c,l) =>{
        c.SendEventAsync(CreateMessage()).ConfigureAwait(false);
        c.SendEventAsync(CreateLogMessage()).ConfigureAwait(false);
        l?.LogDebug($"Sent message: {MessageString}");
        l?.LogDebug($"Sent log message: {LogMessageString}");
    };
    
    internal string BeltAlert => BeltStoppedSeconds > 5 ? "true" : "false";
